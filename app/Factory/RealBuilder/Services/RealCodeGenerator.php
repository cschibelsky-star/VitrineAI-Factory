...
class RealCodeGenerator
{
    public function __construct(
        ...
        protected RealProjectScaffolder $scaffolder,
    ) {
    }
    ...
    public function generate(string $blueprintSlug): array
    {
        ...
        if (! is_array($blueprint)) {
            throw new \RuntimeException("Blueprint inválido: {$blueprintSlug}");
        }
        ...
        $generated = $this->scaffolder->scaffold($base, $blueprint);
        ...
        $report = [
            ...
            'type' => $blueprint['type'] ?? 'custom',
            'status' => 'generated',
            'version' => 'EVOLUCAO_REAL_002',
            'files_count' => count($generated),
            'files' => array_values(array_map(fn ($file) => str_replace($base . '/', '', $file), $generated)),
    ...
    protected function generateModule(string $base, array $module, int $index): array
    {
        ...
        $files[] = $this->write($base . "/app/Filament/Resources/{$resource}/Pages/List" . $this->names->studly($slug) . ".php", $this->listPageCode($model, $resource, $slug));
        $files[] = $this->write($base . "/app/Filament/Resources/{$resource}/Pages/View{$model}.php", $this->viewPageCode($model, $resource));
        $files[] = $this->write($base . "/database/seeders/{$model}Seeder.php", $this->seederCode($model, $module));
    ...
    protected function modelCode(array $module): string
    {
        ...
        foreach (($module['fields'] ?? []) as $field) {
            ...
            if (in_array($type, ['date', 'datetime'], true)) {
                ...
                $casts[] = "'{$name}' => 'boolean'";
            } elseif (in_array($type, ['decimal', 'money'], true)) {
    ...
    protected function migrationCode(array $module): string
    {
        ...
        foreach (($module['fields'] ?? []) as $field) {
            ...
            if ($type === 'foreignId' || str_ends_with($name, '_id')) {
                $table = $field['related_table'] ?? $this->names->relatedTable($name);
                $lines[] = "\$table->foreignId('{$name}')->nullable()->constrained('{$table}')->nullOnDelete();";
            } elseif ($type === 'text') {
                $lines[] = "\$table->text('{$name}'){$nullable};";
            } elseif ($type === 'longText') {
                $lines[] = "\$table->longText('{$name}'){$nullable};";
            } elseif ($type === 'date') {
                $lines[] = "\$table->date('{$name}'){$nullable};";
            } elseif ($type === 'datetime') {
                $lines[] = "\$table->dateTime('{$name}'){$nullable};";
            } elseif (in_array($type, ['decimal', 'money'], true)) {
                $lines[] = "\$table->decimal('{$name}', 12, 2){$nullable};";
            } elseif ($type === 'integer') {
                $lines[] = "\$table->integer('{$name}'){$nullable};";
            } elseif ($type === 'boolean') {
                $lines[] = "\$table->boolean('{$name}')->default(false);";
            } elseif ($type === 'json') {
                $lines[] = "\$table->json('{$name}'){$nullable};";
            } else {
                $lines[] = "\$table->string('{$name}'){$nullable}{$unique};";
            }
        }
...
PHP;
    ...
    protected function resourceCode(array $module): string
    {
        ...
        $model = $this->names->modelName($slug);
        $resource = $this->names->resourceName($slug);
        $label = $module['label'] ?? $this->names->headline($slug);
        ...
        foreach (($module['fields'] ?? []) as $field) {
            $name = $field['name'];
            $headline = $this->names->headline($name);
            $type = $field['type'] ?? 'string';
            $required = !($field['nullable'] ?? true) ? '->required()' : '';

            if ($type === 'foreignId' || str_ends_with($name, '_id')) {
                $rel = $this->names->relationName($name);
                $formFields[] = "Forms\\Components\\Select::make('{$name}')->label('{$headline}')->relationship('{$rel}', 'nome')->searchable()->preload(){$required}";
                $tableColumns[] = "Tables\\Columns\\TextColumn::make('{$rel}.nome')->label('" . $this->names->headline($rel) . "')->searchable()->toggleable()";
            } elseif (in_array($type, ['text', 'longText'], true)) {
                $formFields[] = "Forms\\Components\\Textarea::make('{$name}')->label('{$headline}')->columnSpanFull(){$required}";
                $tableColumns[] = "Tables\\Columns\\TextColumn::make('{$name}')->label('{$headline}')->limit(40)->toggleable()";
            } elseif ($type === 'date') {
                $formFields[] = "Forms\\Components\\DatePicker::make('{$name}')->label('{$headline}'){$required}";
                $tableColumns[] = "Tables\\Columns\\TextColumn::make('{$name}')->label('{$headline}')->date('d/m/Y')->sortable()";
            } elseif ($type === 'datetime') {
                $formFields[] = "Forms\\Components\\DateTimePicker::make('{$name}')->label('{$headline}'){$required}";
                $tableColumns[] = "Tables\\Columns\\TextColumn::make('{$name}')->label('{$headline}')->dateTime('d/m/Y H:i')->sortable()";
            } elseif (in_array($type, ['decimal', 'money'], true)) {
                $formFields[] = "Forms\\Components\\TextInput::make('{$name}')->numeric()->label('{$headline}'){$required}";
                $tableColumns[] = "Tables\\Columns\\TextColumn::make('{$name}')->label('{$headline}')->money('BRL')->sortable()";
            } elseif ($type === 'boolean') {
                $formFields[] = "Forms\\Components\\Toggle::make('{$name}')->label('{$headline}')";
                $tableColumns[] = "Tables\\Columns\\IconColumn::make('{$name}')->label('{$headline}')->boolean()";
            } else {
                $formFields[] = "Forms\\Components\\TextInput::make('{$name}')->label('{$headline}'){$required}";
                $tableColumns[] = "Tables\\Columns\\TextColumn::make('{$name}')->label('{$headline}')->searchable()->sortable()->toggleable()";
            }
        }

        $formCode = implode(",\n                    ", $formFields) ?: "Forms\\Components\\TextInput::make('nome')->required()";
        $columnsCode = implode(",\n                    ", $tableColumns) ?: "Tables\\Columns\\TextColumn::make('id')->sortable()";
        $listPage = 'List' . $this->names->studly($slug);
...
PHP;
    ...
    protected function seederCode(string $model, array $module): string
    {
        $fields = [];
        foreach (($module['fields'] ?? []) as $field) {
            $name = $field['name'];
            ...
            $fields[$name] = match ($type) {
                'text', 'longText' => 'Registro inicial gerado pela Factory.',
                'date' => now()->toDateString(),
                'datetime' => now()->toDateTimeString(),
                'decimal', 'money' => 0,
                'integer' => 0,
                'boolean' => true,
                default => $this->names->headline($name) . ' exemplo',
            };
        }

        $arrayCode = var_export($fields, true);
...
class {$model}Seeder extends Seeder
{
    ...
    {
        {$model}::query()->firstOrCreate({$arrayCode});
    }
}
