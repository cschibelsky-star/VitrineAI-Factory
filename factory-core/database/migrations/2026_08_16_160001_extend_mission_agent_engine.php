...
return new class extends Migration
{
    ...
    {
        Schema::table('factory_agents', function (Blueprint $table) {
            $table->string('role', 80)->nullable()->after('specialty');
            $table->json('skills')->nullable()->after('description');
            $table->json('metadata')->nullable()->after('agent_dna');
        });

        Schema::table('factory_missions', function (Blueprint $table) {
            $table->json('result')->nullable()->after('mission_dna');
            $table->timestamp('started_at')->nullable()->after('result');
    ...
    public function down(): void
    {
        ...
        Schema::table('factory_missions', function (Blueprint $table) {
            $table->dropColumn(['result', 'started_at', 'finished_at']);
        });

        Schema::table('factory_agents', function (Blueprint $table) {
            $table->dropColumn(['role', 'skills', 'metadata']);
        });
    }
};
